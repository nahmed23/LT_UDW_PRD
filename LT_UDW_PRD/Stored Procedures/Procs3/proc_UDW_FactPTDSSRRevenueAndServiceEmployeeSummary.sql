...
SELECT FactGoal.goal_effective_dim_date_key AS GoalEffectiveDimDateKey,
       FactGoal.dim_reporting_hierarchy_key AS DimReportingHierarchyKey,
       Sum(FactGoal.goal_dollar_amount) AS GoalDollarAmount,
	   FactGoal.dim_club_key AS DimClubKey,     -------- Name Change
	   Club.club_id AS ClubID,
...
FROM [marketing].[v_fact_revenue_goal] FactGoal
 JOIN #PTDSSRCategories PTDSSRCategories
   ON FactGoal.dim_reporting_hierarchy_key = PTDSSRCategories.DimReportingHierarchyKey
 JOIN [marketing].[v_dim_club] Club
   ON FactGoal.dim_club_key = Club.dim_club_key
WHERE PTDSSRCategories.EffectiveDimDateKey <= @ReportDateDimDateKey
  AND PTDSSRCategories.ExpirationDimDateKey > @ReportDateDimDateKey
...
GROUP BY FactGoal.goal_effective_dim_date_key,
       FactGoal.dim_reporting_hierarchy_key,
	   FactGoal.dim_club_key,
	   Club.club_id,
	   PTDSSRCategories.PTDSSRCategory,
	   PTDSSRCategories.PTDSSRRowLabel
...
WHERE (MRP.activation_dim_date_key <= Dates.DimDateKey AND IsNull(MRP.termination_dim_date_key,'-998') = '-998')
  ...
  DROP TABLE #PackageSales; 
   ...
   SELECT FactPackage.reporting_dim_club_key AS DimClubKey,
          DimClub.MMSClubID,
          SalesCommissionEmployee.employee_id AS EmployeeID,					------ missing employe on e-commerce sourced packages defect UDW-10771	
...
SELECT @ReportDateDimDateKey AS ReportDateDimDateKey,
 ...
 Order by MMSClubID, PTDSSRCategory,PTDSSRRowLabel,EmployeeID
 */
 ...
 LEFT JOIN [marketing].[v_dim_reporting_hierarchy] DimHierarchy
   ...
   --AND DimHierarchy.ActiveInd = 'Y'
 LEFT JOIN #PTDSSRCategories PTDSSRCategories
   ON DimHierarchy.dim_reporting_hierarchy_key = PTDSSRCategories.DimReportingHierarchyKey
