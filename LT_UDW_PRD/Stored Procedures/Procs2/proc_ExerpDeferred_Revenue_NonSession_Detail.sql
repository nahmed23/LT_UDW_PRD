...
SELECT c.club_code,
      ...
      case when fact_exerp_subscription_participation.delivered_dim_date_key > @EndDimDateKey
                 then 1
            else 0
  ...
  INTO #deferred_revenue
  from marketing.v_fact_exerp_subscription_participation fact_exerp_subscription_participation /* Brian's Magic Table*/
  ...
  INNER JOIN #WDCostCenterPromptList cc 
	on mmsp.workday_cost_center = cc.item or cc.item = 'All Cost Centers'
INNER JOIN #WDOfferingPromptList ofr 
...
inner join #DimLocationInfo  clist on c.dim_club_key = clist.DimClubKey
  where 1=1
    and fact_exerp_subscription_participation.delivered_dim_date_key >= @StartDimDateKey
    --and fact_exerp_subscription_participation.delivered_dim_date_key <= @EndDimDateKey
