...
WHERE (invoice_dim_date_key >=  @StartDateDimDateKey
      AND invoice_dim_date_key < @AdjustedEndDateDimDateKey)
	OR (refund_dim_date_key >= @StartDateDimDateKey
	  AND refund_dim_date_key < @AdjustedEndDateDimDateKey)
       ...
       CASE WHEN RevenueClub.club_id = 51     ------- Bloomingdale, IL
		...
		CASE WHEN TransactionItem.payment_type = 'AE' and TransactionItem.refund_flag = 'N'
		     THEN (CASE WHEN TransactionBucksAmount > DiscountedItemAmount
						...
						ELSE TransactionAmount - TransactionBucksAmount + Tax + OrderShippingAmount
						END )
			 ELSE 0
			 END Capture_amex,
